...
OUT_DIR         = pathlib.Path("out");  IMG_DIR = OUT_DIR / "imgtestNEW"
...
SEARCH_AOI = {"type":"Polygon","coordinates":[[
    ...
    [-52.336121,70.788206]]]}
# DATE_RANGE = "2017-04-03T00:00:00Z/2022-01-20T23:59:59Z"
DATE_RANGE = "2025-05-06T00:00:00Z/2025-06-25T00:00:00Z"


# ───────── thresholds ──────────────────────────────────────
# (see long comment block in original for explanations)
...
while True:
    ...
    # log Sentinel-2 processing baseline
    baseline = it.properties.get("s2:processing_baseline","N/A")
    logging.info("   processing baseline: %s", baseline)
    ...
    water = (ndwi > NDWI_THR) & ~ice_light & ~ice_solid & \
            ~cloud & ~land & ~nodata

    # ---------- overlay PNG (unchanged) ---------------------
    overlay = overlay_rgb(rgb512, ice_solid, ice_light, water,
                          cloud, land, nodata)
    ...
    unknown = total-(ice_solid.sum()+ice_light.sum()+cnt_water+
                     cnt_cloud+cnt_land+cnt_nodata)
    csv_wr.writerow([
        it.id, ts,
