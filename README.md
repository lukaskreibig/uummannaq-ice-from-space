...
```
   1103 scenes        10 seasons, 2017 to 2026, Sentinel-2 L1C
     36 scripts       one question each, all committed with their artefacts
  ...
  over 4000 lines     documentation, ordered by how much each weakness could change a conclusion
    140 tests         plus four gates that run on every push, one of them `make audit`
```
...
scripts/                # THE ANALYSIS: 36 scripts, one question each; the 22 with a published answer are tabled above
